    ...
    for ax in axes.ravel():
        for h, l in zip(*ax.get_legend_handles_labels()):
            if l not in labels:
                ...
                label2handle[l] = h
    legend_n_col: int = (
        len([x for x in labels if "(Full)" not in x]) if ncol is None else ncol
    )
    fig.legend(
        [label2handle[l] for l in labels],
        labels,
        loc="lower center",
        ncol=legend_n_col,
        fontsize=fontsize,
...
def plot_one_labeling_function(
    df: pd.DataFrame,
    ax: plt.Axes,
    ...
    for m_idx, model in enumerate(models):
        ...
        for h_idx, head in enumerate(heads):
            ...
            df_means_ = df_means[
                (df_means["model"] == model) & (df_means["head"] == head)
            ].sort_values(by="k")
            df_stds_ = df_stds[
                (df_stds["model"] == model) & (df_stds["head"] == head)
            ].sort_values(by="k")
...
def plot_one_task_group_box_plot(
    df: pd.DataFrame,
    ax: plt.Axes,
    task_group: str,
    score: str,
    ...
    df = filter_df(
        df, score=score, task_group=task_group, model_heads=model_heads, ks=[-1]
    )
...
def plot_all_labeling_functions(
    ...
    labeling_functions: List[str] = (
        df_results[df_results["score"] == score]["labeling_function"].unique().tolist()
    )
    for idx, labeling_function in enumerate(labeling_functions):
        sub_tasks: List[str] = (
            df_results[
                (df_results["score"] == score)
                & (df_results["labeling_function"] == labeling_function)
            ]["sub_task"]
            .unique()
            .tolist()
        )
    ...
    plt.savefig(
        os.path.join(path_to_output_dir, f"all_labeling_functions_{score}.png"), dpi=300
    )
    plt.close("all")
    return fig


def plot_all_task_groups(
    df_results: pd.DataFrame,
    score: str,
    path_to_output_dir: str,
    model_heads: Optional[List[Tuple[str, str]]] = None,
    ...
    plt.subplots_adjust(bottom=0.1)
    plt.savefig(
        os.path.join(path_to_output_dir, f"all_task_groups_{score}.png"), dpi=300
    )
    plt.close("all")
    return fig


def plot_all_task_group_box_plots(
    df_results: pd.DataFrame,
    score: str,
    path_to_output_dir: str,
    model_heads: Optional[List[Tuple[str, str]]] = None,
    ...
    for idx, task_group in enumerate(task_groups):
        plot_one_task_group_box_plot(
            df_results, axes.flat[idx], task_group, score, model_heads=model_heads
        )
    ...
    plt.savefig(
        os.path.join(path_to_output_dir, f"all_task_groups_box_{score}.png"), dpi=300
    )
    plt.close("all")
    return fig


def plot_clinicalbert_comparison_by_type(
    df_results: pd.DataFrame,
    score: str,
    path_to_output_dir: str,
    is_x_scale_log: bool = True,
    ...
    clinicalbert_models = [
        model for model in df_results["model"].unique() if "clinicalbert" in model
    ]
    ...
    if df_all_tasks.empty:
        ...
        plt.savefig(
            os.path.join(
                path_to_output_dir, f"clinicalbert_type_comparison_{score}.png"
            ),
            dpi=300,
        )
        plt.close("all")
    ...
    type1_data = df_all_tasks[
        df_all_tasks["model"] == "clinicalbert_type1_clinicalbert_pool"
    ]
    type2_data = df_all_tasks[
        df_all_tasks["model"] == "clinicalbert_type2_clinicalbert_pool"
    ]
    type3_data = df_all_tasks[
        df_all_tasks["model"] == "clinicalbert_type3_clinicalbert_pool"
    ]
    ...
    for cb_type, cb_data, color, label in [
        ...
        (
            ...
            type3_data,
            "#d62728",
            'ClinicalBERT "human_narrative" (type 3) + LR',
        ),
    ]:
        if cb_data.empty:
            continue
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(path_to_output_dir, f"clinicalbert_type_comparison_{score}.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close("all")
...
def plot_clinicalbert_pooling_comparison_mean_all_tasks(
    df_results: pd.DataFrame,
    score: str,
    path_to_output_dir: str,
    cb_type: str = "type3",
    ...
    for pool_idx, (pooling, color, marker) in enumerate(
        zip(pooling_strategies, colors, markers)
    ):
    ...
    ax.set_title(
        f"ClinicalBERT {cb_type.title()} Pooling Strategy Comparison", fontsize=16
    )
    ...
    plt.savefig(
        os.path.join(
            path_to_output_dir,
            f"clinicalbert_{cb_type}_pooling_mean_all_tasks_{score}.png",
        ),
        dpi=300,
        bbox_inches="tight",
    )
...
def plot_detailed_task_embedding_comparison(
    df_results: pd.DataFrame, score: str, path_to_output_dir: str
):

    shared_heads = ["lr_lbfgs", "knn"]
    cb_model = "clinicalbert_type3_clinicalbert_pool"  
    ...
    bars1 = ax.bar(
        ...
        clmbr_best_scores,
        bar_width,
        label="CLMBR (Best Prediction Head)",
        color="steelblue",
        alpha=0.8,
    )
    bars2 = ax.bar(
        ...
        cb_best_scores,
        bar_width,
        label="ClinicalBERT Type3 Custom Pooling (Best Prediction Head)",
        color="lightcoral",
        alpha=0.8,
    )

    for i, (bar1, bar2, clmbr_head, cb_head) in enumerate(
        zip(bars1, bars2, clmbr_best_heads, cb_best_heads)
    ):
    ...
    task_labels = [
        LABELING_FUNCTION_2_PAPER_NAME.get(task, task) for task in valid_tasks
    ]
    ax.set_xticks(x_positions)
    ax.set_xticklabels(task_labels, rotation=45, ha="right")

    category_colors = {
        "operational_outcomes": "lightblue",
        "lab_values": "lightgreen",
        "new_diagnoses": "lightyellow",
    }
        ...
        if category != current_category:
            if current_category is not None:
                ax.axvspan(
                    ...
                    i - 0.5,
                    alpha=0.2,
                    color=category_colors.get(current_category, "lightgray"),
                )
    ...
    for category, positions in category_positions.items():
        ...
        ax.text(
            ...
            bbox=dict(
                ...
                facecolor=category_colors.get(category, "lightgray"),
                alpha=0.7,
            ),
        )
    ...
    plt.savefig(
        os.path.join(
            path_to_output_dir, f"detailed_task_embedding_comparison_{score}.png"
        ),
        dpi=300,
        bbox_inches="tight",
    )
...
def plot_embedding_comparison_lr_only(
    df_results: pd.DataFrame, score: str, path_to_output_dir: str
):
    task_groups: List[str] = list(TASK_GROUP_2_LABELING_FUNCTION.keys())
    ...
    for idx, task_group in enumerate(task_groups):
        ...
        clmbr_data = task_data[
            (task_data["model"] == "clmbr") & (task_data["head"] == lr_head)
        ]

        if not clmbr_data.empty:
            clmbr_grouped = (
                clmbr_data.groupby("k")["value"].agg(["mean", "std"]).reset_index()
            )
            ax.errorbar(
                clmbr_grouped["k"],
                clmbr_grouped["mean"],
                yerr=clmbr_grouped["std"],
        ...
        if not cb_data.empty:
            cb_grouped = (
                cb_data.groupby("k")["value"].agg(["mean", "std"]).reset_index()
            )
            ax.errorbar(
                cb_grouped["k"],
                cb_grouped["mean"],
    ...
    plt.savefig(
        os.path.join(path_to_output_dir, f"embedding_comparison_lr_only_{score}.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close("all")
...
def main():
    ...
    for score in supported_scores:
        ...
        plot_clinicalbert_pooling_comparison_mean_all_tasks(
            df_results, score, args.path_to_output_dir, "type3"
        )
        plot_detailed_task_embedding_comparison(
            df_results, score, args.path_to_output_dir
        )
